...
class FRCNN(object):
    ...
    # 加载不同的参数
    _defaults = {
        "model_path": 'model_data/original.pth',
        # "model_path": 'model_data/imagnet.pth',
        # "model_path": 'model_data/mask rcnn.pth',
    ...
    def detect_image(self, image, crop=False, count=False):
        ...
        #  绘制第一阶段图片
        # box = rois[0,:,:].cpu().numpy()

        # for i in range(top_boxes.shape[0]):
        #     top = max(0, np.floor(box[i,0]).astype('int32'))
        #     left = max(0, np.floor(box[i,1]).astype('int32'))
        #     bottom = min(image.size[1], np.floor(box[i,2]).astype('int32'))
        #     right = min(image.size[0], np.floor(box[i,3]).astype('int32'))
